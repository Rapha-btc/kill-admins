...
;;             (match (contract-call? PEPE-CONTRACT remove-principal-from-role sin beelzebub) ;; u1 is the sin of it all and that's why we're alive
;;         success (ok true)
...
(define-public (integrate-the-evil (sin uint) (beelzebub principal))
  (let 
  (
    (result (try! (contract-call? PEPE-CONTRACT remove-principal-from-role sin beelzebub)))
  )
   (ok result)
    )
  )
